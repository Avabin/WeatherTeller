...
public class SettingsRepositoryTests
{
    ...
    public async Task GetSettingsAsync_ShouldReturnSettings_WhenSettingsExist()
    {
        ...
        var settings = new Settings.Settings { Id = userName, ApiKey = "test-api-key" };
        _settingsDataSource.Contains(Arg.Is<Id<string>>(i => i.Value.Equals(userName)))
    ...
    public async Task GetSettingsAsync_ShouldReturnNull_WhenSettingsDoNotExist()
    {
        // Arrange
        var userName = Environment.UserName;
        _settingsDataSource.GetById(userName).Returns(Observable.Empty<Settings.Settings>());
    ...
    public async Task CreateSettingsAsync_ShouldAddSettingsAndPublishNotification()
    {
        ...
        // Assert
        await _settingsDataSource.Received(1)
            .Add(Arg.Is<Settings.Settings>(s => s.Id == "test-id" && s.ApiKey == "test-api-key"));
        await _mediator.Received(1).Publish(Arg.Is<SettingsChangedNotification>(n => n.After == settings),
    ...
    public async Task UpdateSettingsAsync_ShouldUpdateSettingsAndPublishNotification_WhenSettingsExist()
    {
        // Arrange
        var userName = Environment.UserName;
        var existingSettings = new Settings.Settings { Id = userName, ApiKey = "old-api-key" };
        _settingsDataSource.Contains(Arg.Is<Id<string>>(i => i.Value.Equals(userName)))
            .Returns(Observable.Return(true));
        ...
        // Assert
        await _settingsDataSource.Received(1).ReplaceOne(Arg.Is<Id<string>>(userName),
            Arg.Is<Settings.Settings>(s => s.ApiKey == "new-api-key"));
        await _mediator.Received(1).Publish(
            Arg.Is<SettingsChangedNotification>(n =>
                n.After.ApiKey == "new-api-key" && n.Before!.ApiKey == "old-api-key"),
            Arg.Any<CancellationToken>());
    }
    ...
    public async Task UpdateSettingsAsync_ShouldNotUpdateSettings_WhenSettingsDoNotExist()
    {
        // Arrange
        var userName = Environment.UserName;
        _settingsDataSource.GetById(Arg.Is<Id<string>>(i => i.Value.Equals(userName)))
            .Returns(Observable.Empty<Settings.Settings>());
        Func<SettingsModel, SettingsModel> update = s => s with { ApiKey = "new-api-key" };
        ...
        // Assert
        await _settingsDataSource.DidNotReceive().ReplaceOne(Arg.Any<Id<string>>(), Arg.Any<Settings.Settings>());
