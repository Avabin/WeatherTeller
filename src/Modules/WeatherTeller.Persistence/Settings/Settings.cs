...
public record Settings : IIdentifiable<string>, IComparable<Settings>, IComparable
{
    ...
    public int CompareTo(object? obj)
    {
        ...
        return obj is Settings other
            ? CompareTo(other)
            : throw new ArgumentException($"Object must be of type {nameof(Settings)}");
    }
    ...
    public int CompareTo(Settings? other)
    {
        ...
        return locationComparison != 0
            ? locationComparison
            : string.Compare(ApiKey, other.ApiKey, StringComparison.Ordinal);
    }

    public Id<string> Id { get; init; }

    public static bool operator <(Settings? left, Settings? right) =>
        Comparer<Settings>.Default.Compare(left, right) < 0;
