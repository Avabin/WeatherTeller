...
internal partial class ConfigurationWizardViewModel : ViewModelBase, IRoutableViewModel, IActivatableViewModel
{
    ...
    [Reactive] public int CurrentPageIndex { get; set; } = 0;
    ...
    bool configureLocation, bool configureApiKey)
    {
        ...
        if (configureLocation1)
        {
            Pages.Add(configureLocationViewModel);
        }
        
        if (configureApiKey1)
        {
            Pages.Add(configureApiKeyViewModel);
        }
        ...
        if (!hasPages)
        {
            throw new ArgumentException("At least one page must be configured");
        }
