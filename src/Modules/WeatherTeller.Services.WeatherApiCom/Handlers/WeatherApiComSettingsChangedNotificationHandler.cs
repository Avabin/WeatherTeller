...
internal class WeatherApiComSettingsChangedNotificationHandler(
    IWeatherApiComClient weatherApiComClient,
    ILogger<WeatherApiComSettingsChangedNotificationHandler> logger)
    : INotificationHandler<SettingsChangedNotification>
{
    ...
    public async Task Handle(SettingsChangedNotification notification, CancellationToken cancellationToken)
    {
        ...
        await _weatherApiComClient.SetSettings(settings.ApiKey, settings.Location?.Latitude ?? 0,
